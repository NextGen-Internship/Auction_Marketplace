...
const AuctionDetailsPage: React.FC = () => {
  ...
  const isBidAmountValid = typeof bidAmount === 'number' && bidAmount >= startPrice;
      ...
      <div className="auction-details-container">
        <div className="auction-content">
          ...
          <div className="auction-details">
            ...
            <p className='time-left'>Time Left: <CountdownTimer endDate={new Date(auctionDetails?.endDate)} /> </p>
            {!auctionDetails || !auctionDetails.endDate || new Date(auctionDetails.endDate) > new Date() ? (
              <div className="bid-section">
                <label htmlFor="bidAmount" className='label-bid'>Your Bid: </label>
                <input
                  className={`input-bid ${bidAmount && bidAmount <= auctionDetails?.startPrice ? 'input-bid-invalid' : ''}`}
                  id="bidAmount"
                  value={bidAmount || ''}
                  onChange={(e) => setBidAmount(Number(e.target.value))}
                  placeholder={bidAmount && bidAmount <= auctionDetails?.startPrice ? 'Bid must be higher than start price' : ''}
                />
                
                <button className="bid-button" onClick={handleBidNowClick} disabled={!isBidAmountValid}>
